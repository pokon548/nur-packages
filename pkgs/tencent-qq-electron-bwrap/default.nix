...
in appimageTools.wrapAppImage {
  ...
  extraInstallCommands = ''
    ...
    sed -i '39 i cp ${appimageContents}/workarounds/config.json $(xdg-user-dir)/.config/QQ/versions/' $out/bin/${pname}
    sed -i '40 i mkdir -pv $(xdg-user-dir)/.config/QQ/versions/${version}' $out/bin/${pname}
    sed -i '41 i ln -s ${appimageContents}/resources/app/* $(xdg-user-dir)/.config/QQ/versions/${version}' $out/bin/${pname}\
